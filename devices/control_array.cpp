...
inline bool check_equal(std::array<bool,148> current_line, std::string target_control_signal){
...
void ControlArray::receive_clock(){
    ...
    if (control_registers.size() > 0 /*& control_registers.size() <= CONTROL_ARRAY_SIZE*/) {
        std::array<bool, 148> current_line = control_registers.front();
        control_registers.pop();
        if (check_equal(current_line,"1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111") == true){
            outport = NULL;
            return;
        }
